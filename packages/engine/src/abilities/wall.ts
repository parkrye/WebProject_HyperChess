...
export const wall: AbilityDefinition = {
  id: 'wall',
  name: '성벽',
  description: `수를 놓는 대신, 빈칸 하나에 성벽을 세운다. 성벽은 누구도 들어가거나 지나가거나 공격할 수 없고, 자신의 턴 ${WALL_DURATION}번이 지나면 사라진다. 동시에 ${WALL_LIMIT}개까지 세울 수 있다.`,
  ...
  candidates(state, color) {
    ...
    state.board.forEach((piece, square) => {
      if (!piece && !isWall(state.walls, square)) result.push({ square });
    });
    return result;
  },
  ...
  apply(state, color, params) {
    ...
    // 설치자의 다음 턴 시작부터 줄어들므로, 설치 턴을 포함해 WALL_DURATION턴 동안 남는다
