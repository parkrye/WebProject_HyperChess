...
**`--mirror`를 주면 대진마다 능력 하나를 뽑아 양쪽에 같은 능력을 준다(동족전). 능력을 하나만 지정해도 동족전이 된다. 실력 차이는 동족전으로만 정확히 잰다.** 양쪽 능력이 다르면 능력 상성이 결과를 지배해, 색을 바꿔 두 판을 둬도 1승 1패로 끝나며 실력 차가 묻힌다(같은 변경분이 16종 무작위 대진 51%, 단일 능력 동족전 90%로 나온 적이 있다). 여러 능력을 주는 것은 **밸런스(상성) 측정용**이지 AI 실력 비교용이 아니다.
...
2. 보고서를 대국 기록으로 가져온다
3. 현재 밸런스 버전 이상의 기록이 1500판 이상 모이면 학습해 **후보 가중치**를 낸다 (그 전까지는 수집만)
4. 후보로 **동족전 200판**을 두어 전체 점수율이 기준(기본 50%) 이상이고, **그중 가장 나쁜 능력을 100판 더 두어** 하한(기본 35%) 이상일 때만 `weights.ts`에 적용한다. 미달이면 되돌린다. 적용된 가중치는 다음 사이클 대국에 바로 쓰인다
...
> **왜 대국으로 판정하는가 — 검증 오차 감소는 기력 향상을 뜻하지 않는다.** 텍셀 튜닝의 목적함수는 "기록된 대국의 승패를 맞히는 것"이지 "잘 두는 것"이 아니라, 둘은 갈라진다. 실제로 검증 오차가 0.12682 → 0.12659로 줄어 자동 적용된 가중치가 동족전에서 telekinesis 42%·heir 36%로 전반 회귀한 적이 있다(2026-09-16). 그 튜닝에서 항목이 바뀌지 않은 능력도 같이 회귀해, 공통 가중치가 나쁜 방향으로 옮겨진 것이 대조군으로 확인됐다.
>
> 그래서 `learn` 루프는 오차가 아니라 **동족전 승률**로 적용 여부를 정한다(`tools/gate.ts`). 게이트 없이 돌린 옛 기록(2026-09-16 이전 87사이클 등)은 검증되지 않은 것이므로, 그 결과를 커밋하기 전에는 아래 동족전 비교로 직접 확인해야 한다.

**전체 평균만 보면 능력 하나가 무너진 것을 놓친다.** 실제로 87사이클 학습분은 전체 60%로 통과할 수치였지만 `march`가 22%로 망가져 있었다. 그래서 게이트는 두 단계로 본다.

1. 전체 동족전 200판 → 점수율이 `--gate-min`(기본 0.5) 이상인가
2. 1단계에서 가장 나쁜 능력만 100판 더 → 점수율이 `--gate-floor`(기본 0.35) 이상인가

2단계를 따로 두는 이유는 1단계의 능력별 수치가 능력당 12판뿐이라 노이즈가 크기 때문이다. "1단계에서 어떤 능력도 35% 미만 금지"로 하면 진짜 50%인 능력도 15% 확률로 걸려, 16종 중 하나라도 걸릴 확률이 92%가 되어 멀쩡한 후보가 거의 다 떨어진다. 가장 나쁜 하나만 100판으로 다시 재면 오차가 ±5%p로 줄어, 멀쩡한 능력을 잘못 거를 확률은 약 0.1%, 20%대로 무너진 능력을 놓칠 확률은 약 0.5%가 된다.

게이트 크기는 사이클 시간과 맞바꾼다(기본값 약 12분, 사이클 약 22분). 어느 능력이 2단계에 지목되는지는 `--gate-ms`에 따라 달라진다(탐색 깊이가 달라져 능력별 유불리가 바뀐다).

```bash
npm run learn -w @hyperchess/ai -- --gate-games 300 --gate-ms 500   # 더 엄격하게 (사이클 40분)
npm run learn -w @hyperchess/ai -- --gate-games 0                   # 판정 없이 바로 적용 (옛 동작)
```

후보를 따로 판정할 수도 있다. 떨어지면 `weights.ts`를 원래대로 되돌린다.

```bash
npx tsx packages/ai/tools/gate.ts reports/tune-날짜-시각.json --games 200 --ms 200 --min 0.5 --floor 0.35 --confirm 100
```
