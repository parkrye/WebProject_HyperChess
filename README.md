...
4. 후보로 **동족전 200판**을 두어 점수율이 기준(기본 50%) 이상일 때만 `weights.ts`에 적용한다. 미달이면 되돌린다. 적용된 가중치는 다음 사이클 대국에 바로 쓰인다
...
게이트 크기는 사이클 시간과 맞바꾼다. 기본값(200판 × 200ms, 약 8분)은 오차 ±3.5%p로, 5%p 이상 나빠지는 후보를 약 92% 확률로 거른다.
...
```bash
npx tsx packages/ai/tools/gate.ts reports/tune-날짜-시각.json --games 200 --ms 200 --min 0.5
```
